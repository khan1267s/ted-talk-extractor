#!/usr/bin/env python3
"""
Strict TED Talk Processor
Downloads TED Talks and extracts ONLY speaker-on-stage clips (no slides, images, etc.)
"""
...
class StrictTEDTalkProcessor:
    def __init__(self, output_dir: str = "output_clips"):
        """
        Initialize the strict TED Talk processor.
    ...
    def detect_speaker_on_stage(self, frame: np.ndarray) -> Tuple[bool, dict]:
        """
        Strict detection for speaker on stage only.
        ...
        try:
            ...
            # Method 1: MediaPipe pose detection (for full body)
            pose_results = self.pose.process(rgb_frame)
            if pose_results.pose_landmarks:
                detection_info['pose_detected'] = True
                detection_info['person_count'] += 1
                
                # Check if pose is in center/foreground (speaker on stage)
                landmarks = pose_results.pose_landmarks.landmark
                if landmarks:
                    # Get nose position (should be in center for speaker)
            ...
            if face_results.detections:
                for detection in face_results.detections:
                    ...
                    # Check face position and size (should be prominent for speaker)
                    bbox = detection.location_data.relative_bounding_box
                    if bbox.width > 0.1 and bbox.height > 0.1:  # Face should be reasonably large
                        detection_info['confidence'] += 0.3
            
            # Additional checks for speaker-only content
            if detection_info['person_count'] == 1:
                # Check for text/slides in the frame (we want to avoid these)
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                
                # Detect edges (text/slides create many edges)
                edges = cv2.Canny(gray, 50, 150)
                edge_density = np.sum(edges > 0) / (edges.shape[0] * edges.shape[1])
                
                # If too many edges, likely contains text/slides
                if edge_density < 0.1:  # Low edge density = clean speaker shot
                    detection_info['confidence'] += 0.3
                
                # Check for uniform background (stage should be relatively uniform)
                hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
                saturation = hsv[:, :, 1]
                saturation_std = np.std(saturation)
                
                # Low saturation variation indicates clean background
        ...
        # Strict criteria: exactly 1 person with high confidence
    ...
    def analyze_video_segments(self, video_path: str, segment_duration: int = 30) -> List[Tuple[float, float]]:
        """
        Analyze video to find strict speaker-on-stage segments.
        ...
        for start_time in range(0, int(duration), segment_duration):
            ...
            # Sample more frames for stricter analysis
            sample_times = np.linspace(start_time, end_time, 15)  # Sample 15 frames per segment
            ...
            # Check if segment is speaker-only with strict criteria
            speaker_only_count = 0
            total_confidence = 0.0
            
            for frame in segment_frames:
                is_speaker, detection_info = self.detect_speaker_on_stage(frame)
                if is_speaker:
                    speaker_only_count += 1
                    total_confidence += detection_info['confidence']
            
            # Stricter criteria: 80% of frames must be speaker-only with high confidence
            if (len(segment_frames) > 0 and 
                speaker_only_count / len(segment_frames) > 0.8 and
                total_confidence / len(segment_frames) > 0.7):
...
def main():
    """Main function to run the strict TED Talk processor."""
    parser = argparse.ArgumentParser(description="Strict TED Talk Processor - Extract speaker-only clips (no slides/images)")
    ...
    processor = StrictTEDTalkProcessor(args.output_dir)
    ...
    # Print summary
    print("\n" + "="*50)
    print("STRICT PROCESSING SUMMARY")
    print("="*50)
    ...
    print("\nNote: These clips contain ONLY the speaker on stage (no slides/images)")
